...
async def on_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global business_connection_id, owner_user_id

    # Событие подключения/переподключения бота к бизнес-аккаунту.
    if update.business_connection:
        conn = update.business_connection
        business_connection_id = conn.id
        owner_user_id = conn.user.id
        print(f"[connected] owner_user_id={owner_user_id} connection_id={conn.id}")
        return
    ...
    is_me = owner_user_id is not None and msg.from_user and msg.from_user.id == owner_user_id
